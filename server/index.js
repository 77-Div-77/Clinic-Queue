...
io.on('connection', (socket) => {
  ...
  socket.on('add_patient', (data) => {
    ...
    sendSMS(patient.phone, `ClinicQ: You are added to the queue! Your token is #${token}. Check status live here: ${process.env.APP_URL || 'http://localhost:3000'}/patient.html`);
  });
  ...
  socket.on('send_daily_report', async (data) => {
    const { date } = data;
    const targetEmail = 'divyanshughosh05@gmail.com';
    ...
    try {
      ...
      const htmlMsg = `
        <div style="font-family:sans-serif; max-width:600px; margin:0 auto; padding:20px; border:1px solid #ddd; border-radius:8px;">
          <h2 style="color:#2563eb;">ClinicQ Daily Report: ${date}</h2>
